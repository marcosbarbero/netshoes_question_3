...
/**
 * Classe com o algoritmo de resolução da questão 3.</br>
 * </br>
 * Dada uma stream, encontre o primeiro caracter que no se repita no resto da stream. 
 * O termino da leitura da stream deve ser garantido atraves do metodo hasNext(), 
...
public class Question3App {
     ...
     * 
     * @param input Stream a ser analisado.
     * @return primeiro caracter que não possui repetição no stream.
     * @throws CharNotFoundException Exceção indicando que não foi encontrado cenário 
     * que atendesse o requisito.
     */
	...
	while (input.hasNext()) {
	    ...
	    // Se já existe, logo é repetido, portanto é atribuído +1 ao array de controle
	    if (found >= 0) {
		...
		quantidadeCharsRepetidos = Arrays.copyOf(
			quantidadeCharsRepetidos, chars.length + size);
	    }
     ...
     * 
     * @param quantidadeCharsRepetidos Array com a quantidade de ocorrências do caracter (índice do array).
     ...
     * 
     * @param c Caracter a ser pesquisado.
     * @param chars Array com os caracters.
     * @return Posição identificada.
     */
